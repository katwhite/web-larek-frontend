...
Расширяет Component. Отвечает за отображение одной карточки. В конструктор передаётся брокер событий и темплейт, в каталоге это card-catalog, в корзине это card-basket, в модальном окне это card-preview. Устанавливается слушатель клика на карточку для открытия модального окна\
Поля содержат элементы интерфейса ICard\
Методы:
